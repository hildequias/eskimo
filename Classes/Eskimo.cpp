...
void Eskimo::update () {
    ...
    if (_switchShape) {
        SimpleAudioEngine::getInstance()->playEffect("shape.wav");
        if (_state == kStateBox) {
            //switch to circle
            makeCircleShape();
        } else {
...
void Eskimo::makeCircleShape() {
    ...
    this->setDisplayFrame(SpriteFrameCache::getInstance()->getSpriteFrameByName ("player_circle.png"));
...
void Eskimo::makeBoxShape() {
    ...
    _body->CreateFixture(&fixtureDef);
    _state = kStateBox;
    this->setDisplayFrame(SpriteFrameCache::getInstance()->getSpriteFrameByName ("player_box.png"));
